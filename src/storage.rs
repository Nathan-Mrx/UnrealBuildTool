use serde::{Deserialize, Serialize};
    ...
    impl Project {
        ...
        fn get_engine_version(location: &PathBuf) -> String {
            let uproject_content = fs::read_to_string(location).expect("Unable to read uproject file");
            let engine_association: serde_json::Value = serde_json::from_str(&uproject_content).expect("Unable to parse uproject file");
        ...
        fn get_plugins(location: &PathBuf) -> Vec<String> {
            let uproject_content = fs::read_to_string(location).expect("Unable to read uproject file");
            let uproject: serde_json::Value = serde_json::from_str(&uproject_content).expect("Unable to parse uproject file");
            if let Some(plugins) = uproject.get("Plugins") {
                plugins.as_array().unwrap().iter().map(|plugin| {
                    plugin.get("Name").unwrap().as_str().unwrap().to_string()
                }).collect()
            } else {
                Vec::new()
            }
